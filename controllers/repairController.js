  ...
  if (fromOrTo === "from") {
    switch (dateRange) {
      case "quarter":
        ...
        if (isLeapYear && quarterVal !== "first") daysPassed += 1;
        break;
    }
  }
...
const repairController = {
  ...
  getAllRepairs: async function (req, res) {
    ...
    await repairModel
      ...
      .catch((error) => {
        ...
        res
          .status(500)
          .send({ message: "Error occurred while getting all repairs" });
      });
  },
  ...
  getTotalItemQuantityPerTechnician: async function (req, res) {
    ...
    var dateFromString = new Date(
      Math.round((dateFrom - 25569) * 86400 * 1000)
    );
  ...
  getTotalItemQuantityPerItemModel: async function (req, res) {
    ...
    console.log(
      dateToInteger(req.body.dateFrom, "from", dateRange, quarterVal)
    );
    var dateTo = dateToInteger(req.body.dateFrom, "to", dateRange, quarterVal);
        ...
        .then(async (repairItemModel) => {
          ...
          await repairModel
            ...
            .then(async (repairDefect) => {
              ...
              for (i = 0; i < repairItemModel.length; i++) {
                repairTalliedQuantities[i] =
                  repairTalliedQuantities[i].sort(compareNumbers);
              }
