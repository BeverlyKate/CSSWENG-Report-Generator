...
const repairController = {
  ...
  getTotalItemQuantityPerTechnician: async function (req, res) {
    ...
    if (technician == "default") {
      ...
      await repairModel
        .find({})
        .distinct("repairTechnician1")
        .then(async (repairTechnician1) => {
          ...
          await repairModel
            .find({})
            .distinct("repairTechnician2")
            .then(async (repairTechnician2) => {
              ...
              await repairModel
                .find({
                  ...
                  repairDate: { $gte: dateFrom, $lte: dateTo },
                })
                .then((repair) => {
                  // console.log(repair);
                  ...
                  for (i = 0; i < distinctArray.length; i++) {
                    ...
                    for (j = 0; j < repair.length; j++) {
                      ...
                      //temporary int
                      if (
                        distinctArray[i] == repair[j].repairTechnician1 ||
                        distinctArray[i] == repair[j].repairTechnician2
                      ) {
                        console.log("hatdog");
                  ...
                  res.render("TIQPT", {
                    ...
                    repairTechnician1: repairTechnician1,
                    repairTechnician2: repairTechnician2,
                    repairTalliedQuantities: repairTalliedQuantities,
                ...
                .catch((error) => {
                  console.log(
                    "Finding repairModel repairTechnician 1 repairTechnician 2 error: " +
                      error
                  );
                  const errorMessage =
                    "Finding repairModel repairTechnician 1 repairTechnician 2 error";
            ...
            .catch((error) => {
              console.log(
                "Finding repairModel repairTechnician 2 error: " + error
              );
              const errorMessage =
                "Finding repairModel repairTechnician 2 error";
              res.render("TIQPT", { error: errorMessage });
            });
        })
        .catch((error) => {
      ...
      await repairModel
        .find({
          $or: [
            { repairTechnician1: technician },
            { repairTechnician2: technician },
          ],
          repairDate: { $gte: dateFrom, $lte: dateTo },
        })
        .then((repair) => {
          ...
          for (i = 0; i < repair.length; i++) {
            ...
            if (
              technician == repair[i].repairTechnician1 ||
              technician == repair[i].repairTechnician2
            ) {
              console.log("hatdog");
        ...
        .catch((error) => {
          console.log(
            "Finding repairModel repairTechnician 1 or repairTechnician 2 error" +
              error
          );
          const errorMessage =
            "Finding repairModel repairTechnician 1 or repairTechnician 2 error";
    ...
    await repairModel
      ...
      .then((repair) => {
        ...
        for (i = 0; i < repair.length; i++) {
          ...
          if (
            (technician == repair[i].repairTechnician1 &&
              itemModel == repair[i].repairItemModel) ||
            (technician == repair[i].repairTechnician2 &&
              itemModel == repair[i].repairItemModel)
          ) {
            tempInt += repair[i].repairQuantity;
      ...
      .catch((error) => {
        console.log(
          "Finding repairModel repairTechnician 1 or repairTechnician 2 with item model, status, and date error: " +
            error
        );
        const errorMessage =
          "Finding repairModel repairTechnician 1 or repairTechnician 2 with item model, status, and date error";
    ...
    if (technician == "default") {
      ...
      await repairModel
        ...
        .then(async (repairTechnician1) => {
          ...
          await repairModel
            ...
            .then(async (repairTechnician2) => {
              ...
              await repairModel
                ...
                .catch((error) => {
                  console.log(
                    "Finding repairModel repair technician 1, repair technician 2, and repair date error: " +
                      error
                  );
                  const errorMessage =
                    "Finding repairModel repair technician 1, repair technician 2, and repair date error";
            ...
            .catch((error) => {
              console.log(
                "Finding repairModel repair technician 2 error: " + error
              );
              const errorMessage =
                "Finding repairModel repair technician 2 error";
              res.render("AWDPT", { error: errorMessage });
            });
          ...
          res.render("AWDPT", {
            date: req.body.dateFrom,
            repairTechnician: distinctArray,
            repairAverageWorkingDays: repairAverageWorkingDays,
            notDefault: false,
          });
        })
        .catch((error) => {
          console.log(
            "Finding repairModel repair technician 1 error: " + error
          );
      ...
      await repairModel
        ...
        .catch((error) => {
          console.log(
            "Finding repairModel repairTechnician 1 or repairTechnician 2 with date error: " +
              error
          );
          const errorMessage =
            "Finding repairModel repairTechnician 1 or repairTechnician 2 with date error";
  ...
  getTotalItemQuantityPerItemModel: async function (req, res) {
    ...
    if (category1 == "default") {
      ...
      await repairModel
        .find({})
        .distinct("repairItemModel")
        .then(async (repairItemModel) => {
    ...
    if (itemModel == "default") {
      ...
      await repairModel
        ...
        .then(async (repairItemModel) => {
          ...
          await repairModel
            .find({})
            .distinct("repairDefect")
            .then(async (repairDefect) => {
              ...
              await repairModel
                ...
                .then((repair) => {
                  ...
                  for (let i = 0; i < repairDefect.length; i++) {
                    ...
                    for (let j = 0; j < repairItemModel.length; j++) {
                      ...
                      for (let k = 0; k < repair.length; k++) {
                        ...
                        if (
                          repairItemModel[j] == repair[k].repairItemModel &&
                          repairDefect[i] == repair[k].repairDefect
                        ) {
                          console.log(
                            "repair k defect =" + repair[k].repairDefect
                          );
                  ...
                  repairTalliedQuantities =
                    repairTalliedQuantities.sort(compareNumbers);
                  console.log(
                    "tallied = " + JSON.stringify(repairTalliedQuantities)
                  );
                  //Send to hbs template used
                  res.render("TDPM", {
                    category: category1,
                    ...
                    repairItemModel: repairItemModel,
                    repairDefect: repairDefect,
                    repairTalliedQuantities: JSON.stringify(
                      repairTalliedQuantities
                    ),
                    repairTalliedQuantitiesArray: repairTalliedQuantities,
                  });
                })
                .catch((error) => {
                  console.log(
                    "Cannot find Repair Item Model defect, date, status, and category error: " +
                      error
                  );
                  const errorMessage =
                    "Cannot find Repair Item Model defect, date, status, and category error";
          ...
          await repairModel
            ...
            .then(async (repairDefect) => {
              ...
              await repairModel
                  ...
                  for (let i = 0; i < repairDefect.length; i++) {
                    ...
                    for (let j = 0; j < repairItemModel.length; j++) {
                      ...
                      for (let k = 0; k < repair.length; k++) {
                        ...
                        //defect, increment the temporary int to count its occurence
                        if (
                          repairItemModel[i] == repair[k].repairItemModel &&
                          repairDefect[j] == repair[k].repairDefect
                        ) {
                          tempInt += repair[k].repairQuantity;
                        }
                      }
                ...
                .catch((error) => {
                  console.log(
                    "Cannot find Repair Item Model defect, date, and status error: " +
                      error
                  );
                  const errorMessage =
                    "Cannot find Repair Item Model defect, date, status error";
                  res.render("TDPM", { error: errorMessage });
                });
            })
            .catch((error) => {
      ...
      await repairModel
        ...
        .then(async (repairItemModel) => {
          ...
          await repairModel
            ...
            .catch((error) => {
              console.log(
                "Cannot find Repair Item Model with date, status, and category error: " +
                  error
              );
              const errorMessage =
                "Cannot find Repair Item Model with date, status, and category error";
      ...
      await repairModel
        ...
        .then(async (repairItemModel) => {
          ...
          await repairModel
            ...
            .catch((error) => {
              console.log(
                "Cannot find Repair Item Model with date, and status error: " +
                  error
              );
              const errorMessage =
                "Cannot find Repair Item Model with date, and status error";
